					...
					if (!empty($logo_info)) {
						...
						if ($logo_info->post_mime_type === 'oembed/svg') {
							...
							if ($logo_info->post_mime_type === 'image/svg+xml') {
								...
								$logo_inline = ot_get_option('_uncode_logo_inline');
								if($logo_inline=="on"){
									$logoDivInner .= '<div alt="logo" width="'.round($logo_metavalues['width']).'" height="'.round($logo_metavalues['height']).'" class="img-responsive" style="max-width:'.round($logo_metavalues['width']).'px">
										'.file_get_contents($logo_info->guid).'
									</div>';
								}else{
			...
			if ($socials_active === 'on' || $search_active === 'on' || $woo_icon !== '' || $woo_icon_mobile !== '' || $social_html_inner !== '') {
				...
				if ($type === 'menu-overlay-center' && $param === 'menu-overlay-center') {
					...
					if ($search_active === 'on') {
						...
						$search_inner .= 	'<a href="javascript:;"'.(!$vertical ? ' class="trigger-overlay search-icon" data-area="search" data-container="box-container"' : '').'>
			...
			switch ($type) {
				...
				case 'offcanvas_head':
					...
					$this->html .= 					'<div id="logo-container-mobile" class="col-lg-0 logo-container middle">
																	...
																	'.$wpml_content;
																	//if(in_array('administrator',  wp_get_current_user()->roles)==true && $search !== ''){
																		$this->html .= '<div class="nav navbar-nav navbar-nav-search">
																			'.$search.'
																		</div>';
																	//}
																if ( $cta_menu ) {
