...
if (have_posts()):
	...
	if ($content_id === '') {
		...
		$activeClass = "active";
		if($_GET['cat'] && $_GET['cat']!="" && $_GET['cat']!="all"){
			$activeClass = "";
		}
		$blog_page = get_option('page_for_posts');
		$blog_page = get_permalink($blog_page);
		$cat_html = '<li class="listing_text"><a class="'.$activeClass.'" href="'.$blog_page.'" id="all">See all</a></li>';
		...
		foreach($artigoCats as $cats){
			$activeClass = "";
			if($_GET['cat'] && $_GET['cat']==$cats->slug){
				$activeClass = "active";
			}
			
			$cat_html .= '<li class="listing_text"><a href="?cat='.$cats->slug.'" class="'.$activeClass.'" id="'.$cats->slug.'">'.$cats->name.'</a></li>';
		}

		$the_content .= '<div class="noticias_listings">
			<h1 class="text-stroked mobile-hidden table-hidden"><span class="glitch_text">News</span></h1>
			<h1 class="mobile-hidden table-hidden"><span class="glitch_text">News</span></h1>
		...
		$the_content .= '</div>
				<button class="nav nav-prev">
					<svg class="icon icon-navarrow-prev show-for-desktop" viewBox="0 0 408 408">
				...
				<button class="nav nav-next">
					<svg class="icon icon-navarrow-next show-for-desktop" viewBox="0 0 408 408">
	...
	if ($layout === 'sidebar_right' || $layout === 'sidebar_left')
	{
		...
		if (!$with_builder) $the_content = '<div class="post-content"' . $page_custom_width . '>' . uncode_get_row_template($the_content, $limit_width, $limit_content_width, $style, '', false, false, false) . '</div>';
